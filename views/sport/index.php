...
$pageTitle = 'MEGFILMS || СПОРТ';
$link = 'sport';

require_once $_SERVER['DOCUMENT_ROOT']."/header.php";

$collections = $m->getSportOnlyCollections();
$sportTypes = $m->getSportTypes();
$years = $m->getSportYears();
$countries = $m->getSportCountries();
?>
    ...
    <?php if(isset($_POST['sportType']) || isset($_POST['country']) || isset($_POST['year'])){
        $genre = empty($_POST['sportType']) ? null : (int)$_POST['sportType'];
        ...
        $filtered = $m->getFilteredSports( $genre,$country,$year,$orderBy);
        if (count($filtered) > 0){

            require_once './filtered.php';

         } else {
            echo "<h1 style='text-align: center'>Событий по вашим запросам не найдено</h1>";
        }
    } else {
